...
**Changes**:
- Removed legacy hardcoded paths from the old monorepo layout
...
**To**: `/rust-common/rust-retry/.circleci/`

**Reason**: Configuration is specific to the `rust-retry` project, not the workspace root.
...
**Kept**:
- ✅ `/rust-common/rust-retry/.circleci/` (new location)
...
cp -r rust-retry/.circleci /path/to/other-rust-project/

# Copy ci-check script
cp rust-retry/ci-check.sh /path/to/other-rust-project/
...
- cd rust-retry
- cargo build
- checksum "rust-retry/Cargo.lock"
```
