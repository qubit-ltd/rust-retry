...
本目录包含了 `rust-retry` 项目的 CircleCI 持续集成配置。
...
```bash
cargo llvm-cov --package qubit-retry --lcov --output-path coverage.lcov
cargo llvm-cov --package qubit-retry
```
...
```markdown
[![CircleCI](https://img.shields.io/circleci/build/github/qubit-ltd/rust-retry/main?label=build&logo=circleci)](https://circleci.com/gh/qubit-ltd/rust-retry)
```

效果预览：

![CircleCI Badge](https://img.shields.io/circleci/build/github/qubit-ltd/rust-retry/main?label=build&logo=circleci)
...
```markdown
[![codecov](https://codecov.io/gh/qubit-ltd/rust-retry/branch/main/graph/badge.svg)](https://codecov.io/gh/qubit-ltd/rust-retry)
```
...
创建 `rust-retry/.cargo-audit.toml`：
...
curl -X POST \
  ...
  https://circleci.com/api/v2/project/gh/qubit-ltd/rust-retry/pipeline
```
...
- 🐛 Issue：[GitHub Issues](https://github.com/qubit-ltd/rust-retry/issues)
- 💡 讨论：[GitHub Discussions](https://github.com/qubit-ltd/rust-retry/discussions)
